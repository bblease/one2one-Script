...
Token::Token(tok  typ): typ(typ) { }

Token::Token(tok typ, char val): typ(typ), val(val) { }

Token::Token(tok typ, int num): typ(typ), num(num) { }

Token::Token(tok typ, bool bol): typ(typ), bol(bol) { }
...
*/
bool Parser::is_keyword(std::string var){
...
*/
bool Parser::is_ignored(std::string a){
...
*/
void Parser::check_parens(std::vector<Token> p){
...
*/
inline void Parser::eat(std::vector<Token> &p){
	p.erase(p.begin());
}
...
*/
void Parser::eat(tok t, std::vector<Token> &p){
	if (p[0].typ != t){
...
*/
Expression * Parser::parse(std::vector<Token> &p){
	Expression *out;
	tok t = p[0].typ;
...
*/
Program * Parser::create_program(std::string file){
